export class TypesMap {
    private map: Map<string, { originalName: string, path: string | null }> = new Map()
    public customTypes: Map<string, { type: string, references: string[] }> = new Map()
    ...
    private squash () {
      ...
      const getName = (uniqueName: string) => {
        ...
        if (uniqueName === 'string' || uniqueName === 'number' || uniqueName === 'boolean' || uniqueName === 'null') {
          return uniqueName;
        }
