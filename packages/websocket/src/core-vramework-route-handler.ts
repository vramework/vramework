export class CoreVrameworkRouteHandler {
    private subscriptions = new Map<string, Set<(data: unknown) => void>>()

    constructor(private route: string, private sendData: (data: string) => void) {
    }

    public subscribe(method: string | number | symbol, callback: (data: any) => void) {
    ...
    public unsubscribe(method?: string | number | symbol, callback?: (data: any) => void) {
        const method2 = method?.toString()
        if (!method2 && !callback) {
