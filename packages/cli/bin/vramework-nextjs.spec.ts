...
describe('action function - generateNextJsWrapper arguments', () => {
  ...
  beforeEach(() => {
    ...
    getVrameworkCLIConfigStub = sandbox.stub(getVrameworkCLIConfig, 'getVrameworkCLIConfig');
    extractVrameworkInformationStub = sandbox.stub(extractVrameworkInformation, 'extractVrameworkInformation');
    getFileImportRelativePathStub = sandbox.stub(getFileImportRelativePath, 'getFileImportRelativePath');
    generateNextJsWrapperStub = sandbox.stub(nextjsWrapperGenerator, 'generateNextJsWrapper');
    ...
    extractVrameworkInformationStub.resolves({
      ...
      sessionServicesFactories: { 'sessionFactoryFile.ts': ['sessionFactoryVariable'] },
      singletonServicesFactories: { 'singletonFactoryFile.ts': ['singletonFactoryVariable'] }
    });
  ...
  it('should pass correct imports to generateNextJsWrapper', async () => {
    ...
    assert(generateNextJsWrapperStub.calledOnce, 'generateNextJsWrapper should be called once');

    const [, , vrameworkConfigImport, singletonServicesImport, sessionServicesImport] = generateNextJsWrapperStub.firstCall.args;
    ...
    assert.strictEqual(vrameworkConfigImport, expectedVrameworkConfigImport, 'vrameworkConfigImport should match expected import');
    assert.strictEqual(singletonServicesImport, expectedSingletonServicesImport, 'singletonServicesImport should match expected import');
    assert.strictEqual(sessionServicesImport, expectedSessionServicesImport, 'sessionServicesImport should match expected import');
  });
  ...
  it('should use provided vrameworkConfigFile and vrameworkConfigVariable', async () => {
    ...
    assert.strictEqual(vrameworkConfigImport, expectedImport, 'vrameworkConfigImport should use provided file and variable');
  });
  ...
  it('should generate correct import paths with packageMappings', async () => {
    ...
    assert.strictEqual(vrameworkConfigImport, expectedVrameworkConfigImport, 'vrameworkConfigImport should use package mapping');
    assert.strictEqual(singletonServicesImport, expectedSingletonServicesImport, 'singletonServicesImport should use package mapping');
    assert.strictEqual(sessionServicesImport, expectedSessionServicesImport, 'sessionServicesImport should use package mapping');
  });
  ...
  it('should use provided singletonServicesFactoryFile and singletonServicesFactoryVariable', async () => {
    ...
    const [ , , , singletonServicesImport] = generateNextJsWrapperStub.firstCall.args;

    const expectedImport = `import { customSingletonFactoryVariable } from './customSingletonFactoryFile.ts'`;
    assert.strictEqual(singletonServicesImport, expectedImport, 'singletonServicesImport should use provided file and variable');
  });
  ...
  it('should use provided sessionServicesFactoryFile and sessionServicesFactoryVariable', async () => {
    ...
    const [ , , , , sessionServicesImport] = generateNextJsWrapperStub.firstCall.args;

    const expectedImport = `import { customSessionFactoryVariable } from './customSessionFactoryFile.ts'`;
    assert.strictEqual(sessionServicesImport, expectedImport, 'sessionServicesImport should use provided file and variable');
  });
});
