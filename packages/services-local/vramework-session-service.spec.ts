...
describe('VrameworkSessionService', () => {
    ...
    describe('getUserSession', () => {
        ...
        it('should throw MissingSessionError when no credentials are provided and required', async () => {
            await expect(sessionService.getUserSession(true, request)).to.be.rejectedWith(MissingSessionError);
        });
        ...
        it('should throw InvalidSessionError for non-Bearer Authorization header', async () => {
            ...
            await expect(sessionService.getUserSession(true, request)).to.be.rejectedWith(InvalidSessionError);
        });
        ...
        it('should get session from API key', async () => {
            ...
            expect(session).to.deep.equal(mockSession);
            expect(sessionServiceOptions.getSessionForAPIKey!.calledWith('apikey123')).to.be.true;
        });
        ...
        it('should get session from cookie', async () => {
            ...
            expect(session).to.deep.equal(mockSession);
            expect(sessionServiceOptions.getSessionForCookieValue!.calledWith('cookie123', 'session')).to.be.true;
        });
        ...
        it('should transform session if transformer is provided', async () => {
            ...
            expect(sessionServiceOptions.transformSession!.calledWith(mockSession)).to.be.true;
        });
    });
});