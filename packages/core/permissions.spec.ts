...
describe('verifyPermissions', () => {
  ...
  it('should pass when a single permission function returns true', async () => {
    ...
    await expect(verifyPermissions(permissions, services, data, session)).to.be.fulfilled;
    expect(permissions.group1.calledOnceWith(services, data, session)).to.be.true;
  });
  ...
  it('should fail when a single permission function returns false', async () => {
    ...
    await expect(verifyPermissions(permissions, services, data, session)).to.be.rejectedWith(NotPermissionedError);
  });
